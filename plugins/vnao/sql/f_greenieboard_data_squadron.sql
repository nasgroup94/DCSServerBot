-- FUNCTION: public.greenie_board_data_practice(text, text, text, integer, integer)

-- DROP FUNCTION IF EXISTS public.greenie_board_data_practice(text, text, text, integer, integer);

CREATE OR REPLACE FUNCTION public.greenie_board_data_practice(
	server_name text,
	aircraft_type text,
	players_name text DEFAULT '%'::text,
	min_flight_time integer DEFAULT 0,
	return_limit integer DEFAULT 1000)
    RETURNS TABLE(player_name text, server_name text, flightlog_id text, aircraft_type text, case_num integer, points numeric, server_timestamp timestamp) 
    LANGUAGE 'sql'
    COST 100
...
AS $BODY$
							SELECT
							DISTINCT ON (vnao_lsogrades.server_timestamp, vnao_lsogrades.id)
								players.name,
								...
								vnao_lsogrades.server_timestamp
                            FROM
                                vnao_lsogrades
							...
							GROUP BY
								...
								vnao_lsogrades.points
							ORDER BY
								vnao_lsogrades.server_timestamp DESC,
								vnao_lsogrades.id
                            LIMIT
                               $5
$BODY$;
